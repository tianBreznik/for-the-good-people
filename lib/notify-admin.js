...
function getFromAddress() {
    return (
        ...
        (isSmtpConfigured() && !isProduction()
            ? envValue('SMTP_USER')
            : 'Good People Posting <onboarding@resend.dev>')
    );
}
...
async function sendAuthorApplicationNotification({ name, email, message, duplicate = false }) {
    ...
    const info =
        mode === 'resend'
            ? await sendViaResend({ from, to, replyTo: email, subject, text, html })
            : await sendViaSmtp({ from, to, replyTo: email, subject, text, html });
